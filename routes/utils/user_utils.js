...
async function getTreeLastSeens(user_id) {
  const recipes_id = await DButils.execQuery(
    `select * from recipes inner join seens on recipes.recipe_id=seens.recipe_id where user_id='${user_id}' order by seen_date desc limit 3`
  );
  return recipes_id;
}
...
exports.getTreeLastSeens = getTreeLastSeens;
