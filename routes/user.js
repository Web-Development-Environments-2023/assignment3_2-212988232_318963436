...
router.get("/search", async (req, res, next) => {
  try {
    const recipes = await recipes_utils.searchByName(req.query,req.session.user_id);
    if (recipes.length == 0) {
...
router.get("/random", async (req, res, next) => {
  try {
    const recipes = await recipes_utils.randomRecipes(req.query,req.session.user_id);
    res.send(recipes);
  } catch (error) {
    next(error);
  }
...
router.get("/:recipeId", async (req, res, next) => {
  try {
    user_utils.setseen(req.session.user_id,req.params.recipeId);
